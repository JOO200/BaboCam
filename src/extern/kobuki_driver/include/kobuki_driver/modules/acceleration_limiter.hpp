...
public:
  ...
  std::vector<double> limit(const double &vx, const double &wz)
  {
    if( is_enabled ) {
      //get current time
      std::chrono::system_clock::time_point curr_timestamp;
      //get time difference
      std::chrono::system_clock::duration duration = curr_timestamp - last_timestamp;
      //calculate acceleration
      double linear_acceleration = ((double)(vx - last_vx)) / std::chrono::duration_cast<std::chrono::seconds>(duration).count(); // in [m/s^2]
      double angular_acceleration = ((double)(wz - last_wz)) / std::chrono::duration_cast<std::chrono::seconds>(duration).count(); // in [rad/s^2]
      ...
      if( linear_acceleration > linear_acceleration_max )
        command_vx = last_vx + linear_acceleration_max * std::chrono::duration_cast<std::chrono::seconds>(duration).count();
      else if( linear_acceleration < linear_deceleration_max )
        command_vx = last_vx + linear_deceleration_max * std::chrono::duration_cast<std::chrono::seconds>(duration).count();
      else
        command_vx = vx;
      last_vx = command_vx;

      if( angular_acceleration > angular_acceleration_max )
        command_wz = last_wz + angular_acceleration_max * std::chrono::duration_cast<std::chrono::seconds>(duration).count();
      else if( angular_acceleration < angular_deceleration_max )
        command_wz = last_wz + angular_deceleration_max * std::chrono::duration_cast<std::chrono::seconds>(duration).count();
      else
        command_wz = wz;
      last_wz = command_wz;
