...
public:
  ...
  *******************************************/
  void spin();
  void fixPayload(SerialDataBuffer & byteStream);
...
private:
  ...
  **********************/
  ...
  std::mutex command_mutex; // protection against the user calling the command functions from multiple threads
  ...
  ** Signals
  **********************/
  VoidSignal sig_stream_data, sig_controller_info;
  Signal<const VersionInfo> sig_version_info;
  Signal<const std::string> sig_debug, sig_info, sig_warn, sig_error;
  Signal<const std::vector<std::string>> sig_named;
  Signal<Command::Buffer> sig_raw_data_command; // should be const, but pushnpop is not fully realised yet for const args in the formatters.
  Signal<PacketFinder::BufferType> sig_raw_data_stream; // should be const, but pushnpop is not fully realised yet for const args in the formatters.
  Signal<const std::vector<short>> sig_raw_control_command;
};
