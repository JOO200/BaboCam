...
namespace kobuki {
    ...
    public:
        typedef SerialDataBuffer Buffer;
        // typedef < Buffer > BufferStencil;
        ...
        struct Data {
            Data()
                    : command(BaseControl), speed(0), radius(0), request_flags(0),
        ...
        Command (const enum CommandTypes &type, const enum LedNumber &number, const enum LedColour &colour, Command::Data &current_data);
        Command (const enum CommandTypes &type, const DigitalOutput &digital_output, Command::Data &current_data);
        Command (const enum CommandTypes &type, const enum SoundSequences &number, Command::Data &current_data);
        Command GetVersionInfo();
        //static Command SetVelocityControl(DiffDrive& diff_drive);
        Command (const enum CommandTypes &type, const int16_t &speed, const int16_t &radius);
        Command (const enum CommandTypes &cmd_type,
                                  const unsigned char &type,
                                  const unsigned int &p_gain,
                                  const unsigned int &i_gain,
                                  const unsigned int &d_gain);

        Command (const enum CommandTypes &type);
        ...
        bool deserialise(SerialDataBuffer &byteStream) override { return true; } /**< Unused **/
