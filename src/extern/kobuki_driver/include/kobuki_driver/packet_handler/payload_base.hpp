...
public:
   ...
   * serialisation
   */
  virtual bool serialise(SerialDataBuffer & byteStream)=0;
  virtual bool deserialise(SerialDataBuffer & byteStream)=0;
...
protected:
  ...
  template<typename T>
    void buildVariable(T & V, SerialDataBuffer & buffer)
    {
      if (buffer.size() < sizeof(T))
        return;
      V = static_cast<unsigned char>(buffer.snextc());

      unsigned int size_value(sizeof(T));
      for (unsigned int i = 1; i < size_value; i++)
      {
        V |= ((static_cast<unsigned char>(buffer.snextc())) << (8 * i));
      }
    }

  template<typename T>
    void buildBytes(const T & V, SerialDataBuffer & buffer)
    {
      unsigned int size_value(sizeof(T));
      for (unsigned int i = 0; i < size_value; i++)
      {
        buffer.sputc(static_cast<unsigned char>((V >> (i * 8)) & 0xff));
      }
    }
};
 ...
 */
template<>
inline   void payloadBase::buildVariable<float>(float & V, SerialDataBuffer & buffer)
  {
    if (buffer.size() < 4)
      return;
    unsigned int ui;
    ui = static_cast<unsigned char>(buffer.snextc());

    unsigned int size_value(4);
    for (unsigned int i = 1; i < size_value; i++)
    {
      ui |= ((static_cast<unsigned char>(buffer.snextc())) << (8 * i));
    }
...
template<>
inline void payloadBase::buildBytes<float>(const float & V, SerialDataBuffer & buffer)
  {
    if (buffer.size() < 4)
      return;
    unsigned int size_value(4);
    ...
    for (unsigned int i = 0; i < size_value; i++)
    {
      buffer.sputc(static_cast<unsigned char>((ui >> (i * 8)) & 0xff));
    }
  }
